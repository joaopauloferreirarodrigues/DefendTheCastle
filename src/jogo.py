...
def executar_jogo():
    ...
    while rodando:
        ...
        """Disparo da flecha"""
        if pode_atirar(tempo_atual, tempo_ultimo_tiro, INTERVALO_TIRO):
            nova_flecha = criar_flecha(arqueiro["rect"].centerx, arqueiro["rect"].top)
            flechas.append(nova_flecha)
            tempo_ultimo_tiro = tempo_atual

        """Atualiza a posição do morcego e das flechas"""
        atualizar_morcego(morcego)
        atualizar_flechas(flechas)
        flechas = remover_flechas_fora_da_tela(flechas)

        """Aumentar a pontuação quando a flecha acerta o morcego"""
        flechas, acertos = processar_acerto_flechas(flechas, morcego)
        if acertos > 0:
            pontos = calcular_pontos(pontos, PONTOS_POR_INIMIGO * acertos)
            reposicionar_morcego(morcego)

        """Acaba o jogo se o  arqueiro perder todas as vidas"""
